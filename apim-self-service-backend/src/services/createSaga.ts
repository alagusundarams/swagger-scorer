...
export class CreateSagaOrchestrator {
    ...
    private async provisionInApim(context: SagaContext) {
        ...
        const apimService: any = config.useBackendMocks
            ? await import('./apim.service.mock.js')
            : await import('./apim.service.js');
        ...
        try {
            ...
            switch (context.resourceType) {
                case 'product':
                    ...
                    result = await apimService.createProduct(context.payload.name, context.payload, context.environment);
                    break;
                case 'api':
                    result = await apimService.createApi(context.payload.productId, context.payload, context.environment);
                    break;
                case 'named_value':
                    result = await apimService.createNamedValue(context.payload.productId, context.payload, context.environment);
                    break;
                case 'backend':
                    result = await apimService.createBackend(context.payload, context.environment);
                    break;
                default:
