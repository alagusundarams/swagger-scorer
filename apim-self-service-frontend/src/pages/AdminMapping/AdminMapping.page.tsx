        ...
        <MainLayout>
            <div className="max-w-7xl mx-auto px-6 py-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* LEFT: Resource List */}
                    <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden flex flex-col min-h-[700px]">
                        <div className="p-4 border-b border-gray-100 dark:border-slate-700 flex gap-4">
                            <div className="relative flex-1">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">🔍</span>
                                <input
                                    type="text"
                                    placeholder="Search orphaned resources by name, ID or path..."
                                    value={searchTerm}
                                    onChange={e => setSearchTerm(e.target.value)}
                                    className="w-full bg-gray-50 dark:bg-slate-900 border-none rounded-lg pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-blue-500"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col min-w-0">
                            {isLoading ? (
                                <div className="p-20 text-center flex flex-col items-center justify-center flex-1">
                                    ...
                                    <p className="text-gray-400 font-bold text-xs uppercase tracking-widest">Scanning Inventory...</p>
                                </div>
                            ) : (
                                    ...
                                    <table className="w-full text-left text-sm table-fixed min-w-[600px]">
                                        <thead className="bg-gray-50 dark:bg-slate-900 sticky top-0 z-10">
                                            <tr>
                                                ...
                                                <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-[10px] w-auto">Name / Context</th>
                                                <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-[10px] w-24">Type</th>
                                                <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-[10px] w-20">Env</th>
                                                <th className="p-4 font-bold text-gray-500 uppercase tracking-wider text-[10px] w-24">Region</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                            {filteredOrphans.map(orphan => (
                                                ...
                                                >
                                                    ...
                                                    <td className="p-4">
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-sm font-bold truncate text-gray-900 dark:text-white">
                                                                {orphan.details?.displayName || orphan.name}
                                                            </span>
                                                            {orphan.type === 'Product' && (
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); handleOpenMatrix(orphan); }}
                                                                    className="shrink-0 px-2 py-0.5 bg-blue-50 text-blue-600 text-[9px] font-black uppercase rounded hover:bg-blue-100 transition"
                                                                >
                                                                    Matrix
                                                                </button>
                                                            )}
                                                        </div>
                                                        <div className="text-[10px] text-slate-400 font-mono mt-0.5 truncate max-w-[300px]">{orphan.id}</div>
                                                        ...
                                                        <div className="flex flex-wrap items-center gap-2 mt-2">
                                                            ...
                                                            {orphan.details?.managementMode === 'TERRAFORM_MANAGED' ? (
                                                                ...
                                                                <span className={`px-1.5 py-0.5 text-[9px] font-black uppercase rounded shadow-sm border ${(!orphan.details?.gitRepoUrl)
                                                                    ...
                                                                    {(!orphan.details?.gitRepoUrl) ? 'Ghost (Portal)' : 'Portal Managed'}
                                                                </span>
                                                            )}
                                                        </div>
                                                    </td>
                                                    <td className="p-4">
                                                        <span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${orphan.type === 'Product' ? 'bg-indigo-50 text-indigo-700 border border-indigo-100' :
                                                    ...
                                                    <td className="p-4 text-[10px] font-black text-slate-500 uppercase">{orphan.environment}</td>
                                                    <td className="p-4 text-[10px] font-black text-slate-500 uppercase">{orphan.region}</td>
                                                </tr>
                                            ))}
                                                ...
                                                <tr>
                                                    <td colSpan={5} className="p-20 text-center">
                                                        <div className="flex flex-col items-center justify-center opacity-50">
                                                            ...
                                                            <p className="text-xs font-black uppercase tracking-widest text-slate-400">All Resources Mapped</p>
                                                            <p className="text-[10px] text-slate-500 mt-1">No orphaned items match your current filter.</p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            )}
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-900 border-t border-gray-100 dark:border-slate-700 text-[10px] font-black uppercase tracking-widest text-slate-400 flex justify-between">
                            <span>{selectedIds.size} Selected</span>
                            <span>{filteredOrphans.length} Results</span>
                        </div>
                    </div>
