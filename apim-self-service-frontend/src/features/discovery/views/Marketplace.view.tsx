 ...
 * 
 * FEATURES:
 * - Entitlement Filtering: Users ONLY see what they are allowed to see.
 * - Discovery: Search by business domain (simulated via text match).
 * - "Request Access" shortcuts.
 */
export const MarketplacePage = () => {
    ...
    const displayedProducts = useMemo(() => {
        if (!searchTerm.trim()) return accessibleProducts;
        const lowerTerm = searchTerm.toLowerCase();
        return accessibleProducts.filter(p =>
            p.displayName.toLowerCase().includes(lowerTerm) ||
            p.description.toLowerCase().includes(lowerTerm)
        );
    }, [accessibleProducts, searchTerm]);
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    ...
                    <div className="relative max-w-2xl">
                        ...
                        <input
                            ...
                            className="w-full pl-16 pr-6 py-6 bg-white/10 backdrop-blur-md border border-white/20 rounded-[2rem] text-white placeholder-slate-400 font-bold text-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all shadow-2xl"
                        />
                    </div>
                </div>
            </div>
                ...
                {displayedProducts.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {displayedProducts.map(product => (
                            <div
                                ...
                                className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-xl border border-gray-100 dark:border-slate-700/50 hover:scale-[1.02] hover:shadow-2xl transition-all cursor-pointer group flex flex-col h-full"
                            >
                    ...
                    <div className="text-center py-32 opacity-50">
                        ...
                        <p className="text-gray-500 font-medium">
                            {searchTerm ? `No results match "${searchTerm}" within your permitted scope.` : "You currently do not have access to any external service products."}
                        </p>
                    </div>
                )}
            </div>
