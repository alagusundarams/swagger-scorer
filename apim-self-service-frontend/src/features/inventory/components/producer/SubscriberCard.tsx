...
import type { Team, Subscription } from '../../../../shared/types/domain';
import { maskKey } from '../../../../utils/securityUtils';
import { getDaysUntil, formatReadableDate } from '../../../../utils/dateUtils';
                ...
                {subscription.appRegistration ? (
                    <>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${getEnvironmentTheme(subscription.appRegistration.environment).bg} ${getEnvironmentTheme(subscription.appRegistration.environment).text}`}>
                                {subscription.appRegistration.environment}
                            </span>
                            {subscription.appRegistration.secretExpiryDate && (() => {
                                const daysUntilExpiry = getDaysUntil(subscription.appRegistration.secretExpiryDate);
                        ...
                        <div className="text-xs text-gray-500 dark:text-slate-500 flex items-center gap-1 flex-wrap">
                            ...
                            <span>•</span>
                            <span>Subscribed {formatReadableDate(subscription.createdAt)}</span>
                        </div>
                    </>
                ) : (
                    <>
                ...
                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between gap-3 text-[10px]">
                    <div className="flex items-center gap-3 text-gray-600 dark:text-slate-400">
                        {subscription.expirationDate && (() => {
                            const daysUntilExpiry = getDaysUntil(subscription.expirationDate);
                            const isExpiringSoon = daysUntilExpiry <= 30 && daysUntilExpiry > 0;
                            return (
                                <span className={isExpiringSoon ? 'text-orange-600 dark:text-orange-400 font-semibold' : ''}>
                                    {isExpiringSoon && '⚠️ '}
                                    Sub: {isExpiringSoon ? `${daysUntilExpiry}d` : formatReadableDate(subscription.expirationDate)}
                                </span>
                            );
                        })()}
                        {subscription.keysGeneratedAt && (
                            <span>Keys: {formatReadableDate(subscription.keysGeneratedAt)}</span>
                        )}
                        {subscription.lastSyncedAt && (
                            <span className="text-gray-500 dark:text-slate-500">
                                Synced: {formatReadableDate(subscription.lastSyncedAt)}
                            </span>
                        )}
                    </div>
                    <button
