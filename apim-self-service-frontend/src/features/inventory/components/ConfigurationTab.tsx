...
interface NamedValue {
    key: string;
    values: Record<string, string>; // { DEV: '...', PROD: '...' }
    isSecret: boolean;
    scope: 'Product' | 'API' | 'Global';
    context?: string; // e.g., "Order Processing API"
}

// Enhanced Mock Data (Scope Aware)
const MOCK_NAMED_VALUES: NamedValue[] = [
    {
        key: 'BackendUrl',
        values: {
...
export const ConfigurationTab = ({ product }: Props) => {

    // Simulate Smart Scan logic
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-slate-700 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-900/50">
                    ...
                    <div className="flex gap-2 text-xs text-gray-500">
                        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-purple-500"></span> API Level</span>
                        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-orange-500"></span> Product Level</span>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-slate-900/50 text-xs uppercase tracking-widest text-gray-500 font-bold border-b border-gray-100 dark:border-slate-700">
                                <th className="px-6 py-3 w-1/4">Named Value Key</th>
                                {ENVIRONMENTS.map(env => (
                                    <th key={env} className="px-6 py-3">{env}</th>
                                ))}
                                <th className="px-6 py-3 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                            {MOCK_NAMED_VALUES.map((item) => (
                                <tr key={item.key} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {item.scope === 'API' && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">API</span>}
                                            {item.scope === 'Product' && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300">PROD</span>}
                                            {item.scope === 'Global' && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">GLOBAL</span>}
                                        </div>
                                        <div className="text-[10px] text-gray-400 mt-1 italic">
                                            {item.scope === 'API' ? `Drafted in: ${item.context}` : `Context: ${item.context}`}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button
                                            ...
                                            className="opacity-0 group-hover:opacity-100 transition px-3 py-1.5 text-xs font-bold bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-600 text-gray-700 dark:text-white"
                                        >
                                            Edit
                                        </button>
                                    </td>
