import type { API } from '../../../../shared/types/domain';
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            {!identity ? (
                <div className="bg-gray-50 dark:bg-slate-800/50 border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-3xl p-12 text-center">
                    ...
                    <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">No Identity Linked</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all group">
                        <div className="flex items-start justify-between mb-4">
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${identity.type === 'PRODUCT'
                                ? 'bg-emerald-50 text-emerald-600 border border-emerald-100'
                                : 'bg-blue-50 text-blue-600 border border-blue-100'
                                }`}>
                                {identity.type === 'PRODUCT' ? 'SHARED' : 'ISOLATED'}
                            </span>
                        </div>
                        ...
                        <div className="space-y-3 mt-4">
                            <div>
                                <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Client ID</p>
                                <div className="flex items-center space-x-2">
                                    <code className="text-[9px] text-gray-600 dark:text-slate-300 font-mono bg-gray-50 dark:bg-slate-900/50 px-2 py-1 rounded">
                                        {identity.clientId}
                                    </code>
                                    <button
                                        onClick={() => navigator.clipboard.writeText(identity.clientId)}
                            ...
                            {identity.appIdUri && (
                                <div>
                                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">App ID URI</p>
                                    <code className="text-[9px] text-gray-600 dark:text-slate-300 font-mono italic">
                                        {identity.appIdUri}
                                    </code>
                                </div>
                            )}
