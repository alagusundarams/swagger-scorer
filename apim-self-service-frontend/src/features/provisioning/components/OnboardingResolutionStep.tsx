...
export const OnboardingResolutionStep = ({
    ...
    const handleTypeChange = (name: string, type: 'plain' | 'vault') => {
        setVariables((prev: DetectedVariable[]) => prev.map((v: DetectedVariable) => v.name === name ? { ...v, secretType: type } : v));
    };

    const isValueValid = (val: string) => val.length > 0 && new TextEncoder().encode(val).length <= 1024;

    const isValid = variables.every(v => v.isExisting || isValueValid(v.newValue));
    ...
    return (
        <div className="animate-fade-in text-slate-900 dark:text-white p-8 max-w-5xl mx-auto flex flex-col h-full">
            <div className="mb-8 flex justify-between items-end">
                <div>
                    ...
                    <p className="text-slate-500">
                        Define environment-specific values for your placeholders.
                    </p>
                </div>
            </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 dark:bg-slate-900/50 text-xs uppercase font-bold text-slate-500 sticky top-0 z-10 backdrop-blur-md">
                                <tr>
                                    ...
                                    <th className="p-6">Storage Type</th>
                                    <th className="p-6">Value (Max 1024B)</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={v.name} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                        ...
                                        <td className="p-6">
                                            {v.isExisting ? (
                                                ...
                                                <select
                                                    value={v.secretType}
                                                    onChange={(e) => handleTypeChange(v.name, e.target.value as any)}
                                                    className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-1 text-[10px] font-bold outline-none focus:ring-2 focus:ring-purple-500"
                                                >
                                                    <option value="plain">PLAIN TEXT</option>
                                                    <option value="vault">KEYVAULT URL</option>
                                                </select>
                                            )}
                                        </td>
                                        <td className="p-6">
                                            {v.isExisting ? (
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    <input
                                                        type="text"
                                                        placeholder={v.secretType === 'vault' ? "https://kv-vault.vault.azure.net/secrets/..." : "Enter value..."}
                                                        value={v.newValue}
                                                        onChange={e => handleValueChange(v.name, e.target.value)}
                                                        className={`w-full bg-white dark:bg-slate-900 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-purple-500 outline-none transition-all ${!isValueValid(v.newValue) && v.newValue.length > 0 ? 'border-red-500 ring-red-500/20' : 'border-slate-300 dark:border-slate-600'}`}
                                                        autoFocus={variables.indexOf(v) === 0}
                                                    />
                                                    <div className="flex justify-between items-center px-1">
                                                        <span className="text-[10px] text-slate-400">
                                                            {v.secretType === 'vault' ? "Azure KeyVault Reference" : "Managed Secret"}
                                                        </span>
                                                        <span className={`text-[9px] font-bold ${new TextEncoder().encode(v.newValue).length > 1024 ? 'text-red-500' : 'text-slate-400'}`}>
                                                            {new TextEncoder().encode(v.newValue).length} / 1024B
                                                        </span>
                                                    </div>
                                                </div>
                                            )}
            ...
            <div className="flex justify-between items-center mt-8">
                ...
                <button
                    onClick={() => onNext(variables.filter((v: DetectedVariable) => !v.isExisting).map((v: DetectedVariable) => ({ name: v.name, value: v.newValue, secretType: v.secretType })))}
                    disabled={!isValid}
